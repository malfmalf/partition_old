...
transformed(const point2d_t& p,const point2d_t& t,number_t r){
    return translated(rotated(p,r),t);
}
...
number_t 
triangleArea(const point2d_t& p1,const point2d_t& p2,const point2d_t& p3){
    return 0.5*fabs((p1.x-p3.x)*(p2.y-p1.y)-(p1.x-p2.x)*(p3.y-p1.y));
}
...
cPolygon::bisect(const cLine& l,cPolygon& pl1,cPolygon& pl2){
    ...
    /*int */idip2 = -1;
    point2d_t ip1,ip2;
    gMessage += tostring(int(mLines.size())) + " \n";
    gMessage += tostring(length(l.dir()))+ " \n";
    for(size_t i = 0;i<mLines.size();++i){
        if(idip1>=0 && idip2>=0) break;
        gMessage += "Line : ";
        gMessage += tostring(int(i))+"  ";
        gMessage += tostring(length(mLines[i].dir()))+"  ";
        if(idip1<0){
            gMessage +=" P1 = ";
            gMessage += tostring(int(ray_isect(l,mLines[i],&ip1)));
        ...
        if(idip2<0 || idip2==idip1){
            gMessage +=" P2 = ";
            gMessage += tostring(int(ray_isect(ol,mLines[i],&ip2)));
...
number_t 
cPolygon::area(void) const{
    number_t a = 0.0;
    for(size_t i = 2;i<mPoints.size();++i){
        a += triangleArea(mPoints[0],mPoints[i-1],mPoints[i]);
    }
    return a;
}